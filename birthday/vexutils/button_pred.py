...
class _PredView(ui.View):
    ...
    async def interaction_check(self, interaction: Interaction) -> bool:
        ...
        await interaction.response.send_message(
            "Достъп отказан до тази функция.", ephemeral=True
        )
        return False
...
async def wait_for_yes_no(
    ...
    *,
    ...
    """
    view = _PredView(timeout, ctx.author.id)  # type:ignore
    view.add_item(_PredButton(True, ButtonStyle.blurple, "Да"))
    view.add_item(_PredButton(False, ButtonStyle.blurple, "Не"))
