...
class BirthdayCommands(MixinMeta):
    async def setup_check(self, ctx: commands.Context) -> None:
        ...
        if not await self.check_if_setup(ctx.guild):
            await ctx.send(
                "Тази команда не е настроена за този сървър. "
                f"Администратор трябва да изпълни `{ctx.clean_prefix}bdset interactive`, за да започне."
            )
    ...
    async def set(self, ctx: commands.Context, *, birthday: BirthdayConverter):
        ...
        If you use a date in the format xx/xx/xx or xx-xx-xx DD-MM-YYYY is assumed.

        **Examples:**
        - `[p]bday set 24th September`
        - `[p]bday set 24th Sept 2002`
        - `[p]bday set 24/9/2002`
        - `[p]bday set 24-9-2002`
        - `[p]bday set 24-9`
        """
        ...
        stored_birthday = await self.config.member(ctx.author).birthday()
        if stored_birthday["month"] != 0:
            await ctx.send(
                "Вече сте задали рождения си ден. Не може да го промените."
            )
            return
        ...
        if birthday.year != 1 and birthday.year < MIN_BDAY_YEAR:
            await ctx.send(f"Съжалявам но не мога да задам рождения Ви ден преди {MIN_BDAY_YEAR}.")
            return
        ...
        await ctx.send(f"Рождения Ви ден бе зададен на {str_bday}.")
    ...
    async def upcoming(self, ctx: commands.Context, days: int = 7):
        ...
        **Examples:**
        ...
        if days < 1 or days > 365:
            await ctx.send("Трябва да изберете дни между 1 и 365 (ВКЛ.).")
            return
        ...
        async for member_id, member_data in AsyncIter(all_birthdays.items(), steps=50):
            ...
            parsed_bdays[diff.days].append(
                ...
                + (
                    ""
                    if birthday_dt.year == 1
                    else (f" ще направи {next_bday_year - birthday_dt.year}")
                )
            )
        ...
        if len(parsed_bdays) == 0:
            await ctx.send(f"Няма предстоящи рождени дни в следващите {days} дни.")
            return

        sorted_parsed_bdays = sorted(parsed_bdays.items(), key=lambda x: x[0])

        embed = discord.Embed(title="Предстоящи рождени дни", colour=await ctx.embed_colour())

        if len(parsed_bdays) > 25:
            embed.description = "Твърде много рождени дни за показване. Показват се само първите 25."
...
class BirthdayAdminCommands(MixinMeta):
    ...
    async def interactive(self, ctx: commands.Context):
        ...
        await ctx.send("Натиснете отдолу за започване.", view=SetupView(ctx.author, self.bot, self.config))
    ...
    async def settings(self, ctx: commands.Context):
        ...
        table = Table("Име", "Стойност", title="Настройки за рождени дни")
        ...
        async with self.config.guild(ctx.guild).all() as conf:
            ...
            table.add_row("Текстов канал", channel.name if channel else "Каналът е изтрит")

            role = ctx.guild.get_role(conf["role_id"])
            table.add_row("Роля", role.name if role else "Ролята е изтрита")

            if conf["time_utc_s"] is None:
                time = "invalid"
            else:
                time = datetime.datetime.utcfromtimestamp(conf["time_utc_s"]).strftime("%H:%M UTC")
                table.add_row("Час", time)

            table.add_row("Позволение за споменаване на ролята", str(conf["allow_role_mention"]))

            message_w_year = conf["message_w_year"] or "Няма зададено съобщение"
            message_wo_year = conf["message_wo_year"] or "Няма зададено съобщение"

        warnings = "\n"
        if (error := role is None) or (error := role_perm_check(ctx.me, role)):
            if isinstance(error, bool):
                error = "Ролята е изтрита."
            warnings += warning(error + " Това може да предизвика повторение на известия.\n")
        if (error := channel is None) or (error := channel_perm_check(ctx.me, channel)):
            if isinstance(error, bool):
                error = "Текстовият канал е изтрит."
            warnings += warning(error + " Няма да бъдат изпращани известия за рождени дни.\n")
    ...
    async def time(self, ctx: commands.Context, *, time: TimeConverter):
        ...
        **Examples:**
        ...
        m = (
            "Времето е зададено! Съобщения за рождени дни ще бъдат изпращани в"
        ...
        if old is not None:
            ...
            if time > old_dt and time > datetime.datetime.utcnow():
                m += (
                    "\n\nВремето, което зададохте е по-късно от предишното,"
                    " затова съобщение за рожден ден ще бъде изпратено повторно."
                )
    ...
    async def msgwithyear(self, ctx: commands.Context, *, message: str):
        ...
        **Examples:**
        ...
        if len(message) > MAX_BDAY_MSG_LEN:
            await ctx.send(
                f"Това съобщение е твърде дълго! Трябва да е под {MAX_BDAY_MSG_LEN} символа."
            )
        ...
        try:
            ...
            await ctx.send(
                f"Имате образец `{{{e.args[0]}}}`, който е невалиден. Може да използвате само"
                " `{name}`, `{mention}` и `{new_age}` за съобщението с година."
            )
            return
        ...
        await ctx.send("Съобщението е зададено. Ето как ще изглежда, ако ставате на 20:")
        await ctx.send(
    ...
    async def channel(self, ctx: commands.Context, channel: discord.TextChannel):
        ...
        **Example:**
        ...
        if channel.permissions_for(ctx.me).send_messages is False:
            await ctx.send(
                "Не мога да изпращам съобщения в този канал. Моля, разрешете ми да изпращам съобщения в"
                f" {channel.mention}. Канала за рождени дни не е запазен."
            )
            return
        ...
        await ctx.send(f"Канала за рождени зададен до {channel.mention}.")
    ...
    async def role(self, ctx: commands.Context, *, role: discord.Role):
        ...
        **Example:**
        ...
        if ctx.me.top_role < role:
            await ctx.send(f"Не мога да използвам {role.name}, защото е по-висока роля от моята.")
            return
        ...
        await ctx.send(f"Ролята зададена до {role.name}.")
    ...
    async def forceset(
        ...
        **Examples:**
        ...
        """
        if birthday.year != 1 and birthday.year < MIN_BDAY_YEAR:
            await ctx.send(f"Съжалявам, но не мога да задам рождения ден преди {MIN_BDAY_YEAR}.")
            return
        ...
        await ctx.send(f"Рождения ден на {user.name} бе зададен на {str_bday}.")
    ...
    async def forceremove(self, ctx: commands.Context, user: discord.Member):
        ...
        m = await ctx.send(f"Сигурни ли сте? Рождения ден на `{user.name}` ще бъде премахнат.")
        start_adding_reactions(m, ReactionPredicate.YES_OR_NO_EMOJIS)
        check = ReactionPredicate.yes_or_no(m, ctx.author)  # type:ignore
        ...
        await ctx.send(f"Рождения ден на {user.name} беше премахнат.")
    ...
    async def rolemention(self, ctx: commands.Context, value: bool):
        ...
        if value:
            await ctx.send("Споменаването на ролята е разрешено.")
        else:
            await ctx.send("Споменаването на ролята е забранено.")
