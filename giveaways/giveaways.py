...
class Giveaways(commands.Cog):
    ...
    async def draw_winner(self, giveaway: Giveaway):
        ...
        embed = discord.Embed(
            title=f"{f'{winners}х ' if winners > 1 else ''}{giveaway.prize}",
            description=f"Победител(и):\n{txt}",
            color=await self.bot.get_embed_color(channel_obj),
            timestamp=datetime.now(timezone.utc),
        )
        embed.set_footer(
            text=f"Повторно изтегляне: {(await self.bot.get_prefix(msg))[-1]}gw reroll {giveaway.messageid} | Приключи в"
        )
        try:
            await msg.edit(
                content="🎉 Раздаването приключи 🎉",
                embed=embed,
            )
        ...
        if giveaway.kwargs.get("announce"):
            announce_embed = discord.Embed(
                title="Раздаването приключи",
                description=f"Поздравления на {f'{str(winners)} ' if winners > 1 else ''}победител{'ите' if winners > 1 else 'я'} с [{giveaway.prize}]({msg.jump_url}).\n{txt}",
                color=await self.bot.get_embed_color(channel_obj),
            )

            announce_embed.set_footer(
                text=f"Повторно изтегляне: {(await self.bot.get_prefix(msg))[-1]}gw reroll {giveaway.messageid}"
            )
            await channel_obj.send(
                content="Поздравления " + ",".join([x.mention for x in winner_objs])
            ...
            if giveaway.kwargs.get("congratulate", False):
                for winner in winner_objs:
                    with contextlib.suppress(discord.Forbidden):
                        await winner.send(
                            f"Поздравления! Вие спечелихте {giveaway.prize} от раздаването в {guild}!"
                        )
            async with self.config.custom(
                GIVEAWAY_KEY, giveaway.guildid, int(giveaway.messageid)
            ).entrants() as entrants:
    ...
    @app_commands.describe(
        channel="ИД на канала, в който да се проведе раздаването.",
        time="Колко време да трае раздаването.",
        prize="Каква е наградата от раздаването.",
    )
    async def start(
        self,
        ...
        *,
        ...
        embed = discord.Embed(
            title=f"{prize}",
            description=f"\nРеагирайте с 🎉, за да се запишете\n\n**Организирано от:** {ctx.author.mention}\n\nПриключва: <t:{int(end.timestamp())}:R>",
        ...
        if ctx.interaction:
            await ctx.send("Създадено раздаване!", ephemeral=True)
    ...
    @giveaway.command()
    @app_commands.describe(msgid="ИД на съобщението с раздаването.")
    async def reroll(self, ctx: commands.Context, msgid: int):
        ...
        if msgid in self.giveaways:
            return await ctx.send(
                f"Раздаването вече тече. Моля изчакайте да приключи или ръчно го приключете с `{ctx.clean_prefix}gw end {msgid}`."
            )
    ...
    @giveaway.command()
    @app_commands.describe(msgid="ИД на съобщението за раздаването.")
    async def end(self, ctx: commands.Context, msgid: int):
        """End a giveaway."""
        if msgid in self.giveaways:
            if self.giveaways[msgid].guildid != ctx.guild.id:
                return await ctx.send("Раздаването не е намерено.")
            ...
            await ctx.send("Раздаването не е намерено.")

    @giveaway.command(aliases=["adv"])
    @app_commands.describe(
        arguments="Аргументи за раздаването. Вижте `gw explain` за повече информация."
    )
    async def advanced(self, ctx: commands.Context, *, arguments: Args):
        ...
        embed = discord.Embed(
            ...
            description=f"{description}\n\nРеагирайте с {emoji}, за да се запишете\n\n**Организирано от:** {ctx.author.mention}\n\nКрай: <t:{int(end.timestamp())}:R>",
            color=await ctx.embed_color(),
        )
    ...
    @giveaway.command()
    @app_commands.describe(msgid="ИД на съобщението на раздаването.")
    async def entrants(self, ctx: commands.Context, msgid: int):
        ...
        if not giveaway.entrants:
            return await ctx.send("Няма записали се.")
        count = {}
        ...
        for page in pagify(msg, delims=["\n"], page_length=800):
            embed = discord.Embed(
                title="Записали се", description=page, color=await ctx.embed_color()
            )
            embed.set_footer(text=f"Общо записали се: {len(count)}")
            embeds.append(embed)
    ...
    @giveaway.command()
    @app_commands.describe(msgid="ИД на съобщението за редактиране.")
    async def info(self, ctx: commands.Context, msgid: int):
        ...
        giveaway = self.giveaways[msgid]
        winners = giveaway.kwargs.get("winners", 1) or 1
        msg = f"**Записали се**: {len(giveaway.entrants)}\n**Край**: <t:{int(giveaway.endtime.timestamp())}:R>\n"
        ...
        embed.set_footer(text=f"Раздаване ИД #{msgid}")
    ...
    async def _list(self, ctx: commands.Context):
        ...
        if not self.giveaways:
            return await ctx.send("Няма активни раздавания.")
        giveaways = {
            x: self.giveaways[x]
        ...
        for page in pagify(msg, delims=["\n"]):
            embed = discord.Embed(
                title=f"Разване в {ctx.guild}", description=page, color=await ctx.embed_color()
            )
            embeds.append(embed)
        if len(embeds) == 1:
            return await ctx.send(embed=embeds[0])
